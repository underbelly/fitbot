...
// import {isValidWorkTime, getNextWorkout} from "../helpers/ScheduleHelper";
const {
  isValidWorkTime,
  getNextWorkout,
} = require("../helpers/ScheduleHelper");
const { addWorkoutRecordToUser } = require("../helpers/db_methods");
const { workoutMessage } = require("../helpers/Messages");
...
module.exports = function (controller) {
  controller.ready(async () => {
    if (process.env.MYTEAM) {
      ...
      let currentWorkoutTimestamp = moment.now();
      setInterval(() => {
        console.log("hitting");
        if (isValidWorkTime(currentWorkoutTimestamp)) {
          console.log("ready for next one");
          currentWorkoutTimestamp = moment.now();
          currentWorkout = getNextWorkout(currentWorkout);
          const messageBlock = workoutMessage(currentWorkout);
          bot.say(messageBlock);
        } else {
          console.log("not ready yet", currentWorkoutTimestamp);
        }
      }, THIRTY_SECONDS);
      // bot.say('I AM AWOKEN.');
    }
  ...
  controller.on("block_actions", async (bot, message) => {
    // This is where mark complete is registered
    await bot.startPrivateConversation(message.user);
    // TODO: need to get current workout id in here.
    console.log(message.user);
    let workoutID = message.incoming_message.channelData.actions[0].value;
    let workoutAdded = await addWorkoutRecordToUser(message.user, workoutID);
    console.log('added?', workoutAdded);
    if (workoutAdded) {
      await bot.say(`Good Job, you completed a workout!`);
    } else {
      await bot.say(`Easy bucko, you already finished that workout`);
    }
    // await bot.reply(
    //   message,
